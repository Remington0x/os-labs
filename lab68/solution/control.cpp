	...
	while (std::cin >> s >> id) {
		if (s == "create") {
			...
			if (parent_id == -1) {
				...
				mq::init_pair_socket(new_context, new_socket);
				...
				if (fork_id == 0) {
					...
					ok = mq::recieve_msg_wait(reply_info, new_socket);

					node_token_t* token = new node_token_t({ping, id, id});
					node_token_t reply({fail, id, id});
					ok = mq::send_recieve_wait(token, reply, new_socket);
				...
				if (control_node.find(id) != -1) {
					...
					node_token_t reply({fail, id, id});
					if (mq::send_recieve_wait(token, reply, childs[ind].second) and reply.action == success) {
			...
			if (ind != -1) {
				...
				node_token_t reply({fail, id, id});
				bool ok = mq::send_recieve_wait(token, reply, childs[ind].second);
				if (reply.action == destroy and reply.parent_id == id) {
					...
					rc = zmq_ctx_term(childs[ind].first);
					assert(rc == 0);
					mq::init_pair_socket(childs[ind].first, childs[ind].second);
					rc = zmq_bind(childs[ind].second, ("tcp://*:" + std::to_string(BASE_PORT + reply.id)).c_str());
					assert(rc == 0);
				}
				if (ok) {
			...
			if (ind != -1) {
				...
				node_token_t reply({fail, id, id});
				if (mq::send_recieve_wait(token, reply, childs[ind].second)) {
					if (reply.action == success) {
						...
						while (mq::send_recieve_wait(token_back, reply_back, childs[ind].second) and reply_back.action == success) {
			...
			if (ind != -1) {
				...
				for (size_t i = 0; i < text_pattern.size(); ++i) {
					...
					node_token_t reply({fail, id, id});
					if (!mq::send_recieve_wait(token, reply, childs[ind].second) or reply.action != success) {
						ok = false;
						break;
					}
				}
